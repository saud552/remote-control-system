...
class PerformanceMonitor {
    ...
    setupPerformanceMonitoring() {
        ...
        }, this.monitoringSettings.cleanupInterval);
    ...
    analyzePerformance() {
        ...
        // فحص الأداء
        if (avgResponseTime > this.monitoringSettings.maxResponseTime) {
        ...
        if (currentMemory && currentMemory.percentage > this.monitoringSettings.maxMemoryUsage) {
        ...
        if (currentCPU && currentCPU.percentage > this.monitoringSettings.maxCPUUsage) {
    ...
    cleanupOldMetrics() {
        const now = Date.now();
        const cutoff = now - this.monitoringSettings.logRetention;
    ...
    savePerformanceStats(stats) {
        try {
            const statsPath = path.join(__dirname, 'logs', 'performance-stats.json');
